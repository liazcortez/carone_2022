...
export const getServerSideProps = async (context) => {
  ...
  // `https://apicarone.com/api/v1/preowneds/preownedsByMake/${context.params.marca}?sort=index`
  const res = await fetch(
    `${baseURL}/preowneds/preownedsByMake/${context.params.marca}?sort=index`
  );
  ...
  let resV = [];
  let resM = {}
  if(preowneds.data) resV = preowneds.data
  if(make) resM = make
