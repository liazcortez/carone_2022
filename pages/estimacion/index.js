...
const emptyImage = "https://i.pinimg.com/originals/ae/8a/c2/ae8ac2fa217d23aadcc913989fcc34a2.png";
...
const CalcPrice = () => {
    ...
    const handleSubmit = () => {
        if(state.make === '' || state.model === '' || state.year === '') return enqueueSnackbar('Llena todos los comapos', { variant: 'error'});
        ...
        <Box style={{width: '100%', display: 'flex', marginBottom: '1em'}}>
            <Box p={1} minWidth='25.15%'>
                <TextField
                    ...
                    value={state.make}
                    onChange={(e)=>{
                        setState({ ...state, make: e.target.value, makeName:  e.target.options[e.target.options.selectedIndex].getAttribute('name')})
                    ...
                    variant="outlined"
                >
                    <option key={0} value={"-"}>Selecciona alguna marca</option>
                    {
                        makes.map((option) => {
                            if(option._id === "6269ad81e8436b4af342ac57") return false
                            return <option name={capitalCase(option.name)} key={option._id} value={option._id}>{capitalCase(option.name.replace("-", " "))}</option>
                        })
                    }
                </TextField>
            </Box>
            <Box p={1} minWidth='25.15%'>
                <TextField
                    ...
                    value={state.model}
                    onChange={(e)=>{
                        setState({ ...state, model: e.target.value, modelName:  e.target.options[e.target.options.selectedIndex].getAttribute('name')})
                    ...
                    variant="outlined"
                >
                    <option key={0} value={"-"}>Selecciona algún modelo</option>
                    {
                        vehicles.map((option) => <option name={capitalCase(option.model)} key={option._id} value={option._id}>{capitalCase(option.model)}</option>)
                    }
                </TextField>
            </Box>
            <Box p={1} minWidth='25.15%'>
        ...
        <Card  className={(classes.root, classes.hover)}>
            {
                ...
                <CardMedia
                    ...
                    title={`${vehicle && vehicle.make} ${vehicle && vehicle.model} ${vehicle && vehicle.year}`}
                />
            }
            <CardContent>
                {
                vehicle &&
                    <>
                        ...
                        <Box >
                            ...
                            {`${vehicle && vehicle.model ? capitalCase(vehicle.model) :''} ${vehicle.version} ${vehicle.year}`}
                            </Typography>
                        </Box>
                        ...
                        <Typography variant="h6" gutterBottom style={{ fontSize: 17 }}>
                            Precio de Toma &nbsp;
                            <NumberFormat value={( ( prices.promedio - (vehicle.price * 0.2) ) * 0.8).toFixed(0)} displayType={"text"} thousandSeparator={true} prefix={"$"}/>
                        </Typography>
                        )
                        }
                    </>
